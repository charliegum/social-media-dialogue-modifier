...
const DialogueModifierGenerator = () => {
  ...
  const [leetLevel, setLeetLevel] = useState(20);
  const [algoLevel, setAlgoLevel] = useState(20);
  const [emojiLevel, setEmojiLevel] = useState(40);
  const [typoLevel, setTypoLevel] = useState(15);
  const [capitalLevel, setCapitalLevel] = useState(10);
  const [punctuationLevel, setPunctuationLevel] = useState(30);
  ...
  const applyLeetspeak = (word, intensity) => {
  ...
  const applyAlgospeak = (text, intensity) => {
  ...
  const addEmojis = (text, intensity) => {
    if (Math.random() * 100 > intensity) return text;
  ...
  const applyTypos = (text, intensity) => {
    if (Math.random() * 100 > intensity) return text;
  ...
  // Vary capitalization
  const varyCapitalization = (text, intensity) => {
    if (Math.random() * 100 > intensity) return text;
  ...
  // Vary punctuation
  const varyPunctuation = (text, intensity) => {
    if (Math.random() * 100 > intensity) return text;
  ...
  const applyAllModifications = (text) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {comments.slice(0, 5).map((comment, index) => (
              <div key={comment.id} className="border rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <select
                    value={comment.role}
                    onChange={(e) => handleRoleChange(comment.id, e.target.value)}
          ...
          <div className="space-y-4">
            {comments.slice(5, 10).map((comment, index) => (
              <div key={comment.id} className="border rounded-lg p-3">
                <div className="flex justify-between items-center mb-2">
                  <div className="font-semibold">Comment #{comment.id + 5}</div>
                  <select
                    value={comment.role}
                    onChange={(e) => handleRoleChange(comment.id, e.target.value)}
                    className="border rounded p-1"
